...
public class Product {
	...
	public String getProducts() {
		...
		try {
			...
			oRs = oStmt.executeQuery("select p.id,p.name,p.price,p.shortDesc,p.longDesc,cd.companyName from product p inner join companydetail cd on p.companyId=cd.id where p.deleteFlag=0");
			while(oRs.next()) {
